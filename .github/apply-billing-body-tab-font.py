...
body_font_rules = '''      /* Use the tab-title serif face for normal billing UI text. Headings are intentionally excluded. */
      [data-billing-tab] p,
...
old_comment = '      /* Use the tab-title serif face for normal billing UI text while preserving headings. */\n'
if old_comment not in source and body_font_rules not in source:
    source = source[:insert_at] + body_font_rules + source[insert_at:]
else:
    source = source.replace(old_comment, '      /* Use the tab-title serif face for normal billing UI text. Headings are intentionally excluded. */\n', 1)

heading_block = '''      /* Headings keep their existing typography. */
      [data-billing-tab] h1,
      [data-billing-tab] h2,
      [data-billing-tab] h3,
      [data-billing-tab] h4,
...
'''
source = source.replace(heading_block, '', 1)
...
if 'font-family: Georgia, Times, "Times New Roman", serif;' not in source:
    raise SystemExit('Billing serif rule is missing')

if '[data-billing-tab] h1,' in source:
    raise SystemExit('A heading font override still remains')
