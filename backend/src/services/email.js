...
function senderFor(client) {
  const name = String(process.env.SMTP_FROM_NAME || client.business_name || client.name || 'Support').trim();
  const address = String(process.env.SMTP_FROM_EMAIL || process.env.SMTP_USER || '').trim();
  return { name, address };
}
...
async function sendInstallationRequestEmail(client, details) {
  ...
  const company = process.env.SMTP_FROM_NAME || client.business_name || client.name || 'our team';
