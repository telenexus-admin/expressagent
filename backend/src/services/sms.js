...
function resolveConfig(options = {}) {
  ...
  const rawSenderId = options.senderId || client.sms_sender_id || '';
  const encodedSavvy = parseSavvySender(rawSenderId);
  const requestedProvider = normalizeProvider(
    options.provider || client.sms_provider || process.env.SMS_PROVIDER || 'blessed_text'
  );
  const provider = encodedSavvy ? 'savvy' : requestedProvider;
  ...
  return {
    ...
    senderId: encodedSavvy?.senderId || rawSenderId || (isSavvy ? process.env.SAVVY_SENDER_ID : process.env.BLESSED_SENDER_ID),
    partnerId: options.partnerId || client.sms_partner_id || encodedSavvy?.partnerId || process.env.SAVVY_PARTNER_ID,
