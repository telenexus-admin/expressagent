...
const normalizedScript=script.replace(/\\\s*/g,' ').replace(/\s+/g,' ');
assert.ok(normalizedScript.includes('name=nexa-readonly policy=read,test,api'));
assert.ok(normalizedScript.includes('name=nexa-executor policy=ftp,read,write,policy,test,sensitive,api'));
