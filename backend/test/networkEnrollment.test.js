...
assert.ok(plan.stages.some(x=>x.name==='vlan'));assert.ok(plan.stages.some(x=>x.name==='radius_server_registration'));assert.ok(JSON.stringify(plan).includes('router-radius-secret'));assert.ok(!JSON.stringify(plan).includes('shared_secret'));
...
console.log('Phase 3.5 bootstrap, fingerprint, compatibility, plan, redaction, and safety tests passed.');
