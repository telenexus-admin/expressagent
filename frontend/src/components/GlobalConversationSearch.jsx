    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const matches = data.filter((item) => {
          const searchable = `${item.customer_name || ''} ${item.customer_phone || ''} ${item.last_message || ''}`.toLowerCase();
  ...
  return (
    <div ref={containerRef} className="relative hidden md:block w-[330px] xl:w-[405px]">
      <form onSubmit={submit} className="flex bg-white rounded-full px-5 py-3 shadow-sm border border-slate-100 items-center gap-3 focus-within:ring-2 focus-within:ring-[#4b16b5]/15">
        ...
        <input value={query} onChange={(event) => setQuery(event.target.value)} onFocus={() => query.trim().length >= 2 && setOpen(true)} placeholder="Search name, phone or latest message..." className="bg-transparent text-xs text-slate-700 placeholder:text-slate-400 outline-none flex-1" aria-label="Search customer conversations" />
        {query && <button type="button" onClick={() => { setQuery(''); setResults([]); setOpen(false); }} className="text-slate-300 hover:text-slate-500 text-lg leading-none" aria-label="Clear search">×</button>}
      </form>
      ...
      {open && query.trim().length >= 2 && (
        <div className="absolute right-0 left-0 top-[58px] z-50 rounded-[24px] border border-slate-100 bg-white p-2 shadow-2xl shadow-slate-300/40">
          ...
          {loading ? <div className="px-3 py-5 text-xs text-slate-400">Searching...</div> : results.length === 0 ? <div className="px-3 py-5 text-xs text-slate-400">No customer or message found.</div> : (
              ...
              {results.map((item) => {
                ...
                return (
                  <button key={item.id} type="button" onClick={() => openConversation(item.id)} className="w-full rounded-2xl px-3 py-3 text-left hover:bg-[#f5f1ff] transition-colors">
                    <div className="flex items-center justify-between gap-2"><div className="min-w-0 text-xs font-bold text-slate-800 truncate">{item.customer_name || `+${item.customer_phone}`}</div><span className={`text-[9px] font-bold rounded-full px-2 py-1 shrink-0 ${status.style}`}>{status.text}</span></div>
                    <div className="mt-1 flex items-center justify-between gap-3 text-[11px] text-slate-400"><span className="truncate">+{item.customer_phone}</span><span className="shrink-0">{shortTime(item.last_message_at)}</span></div>
          ...
          {results.length > 0 && <button type="button" onClick={submit} className="mt-2 w-full border-t border-slate-100 py-3 text-xs font-bold text-[#4b16b5] hover:bg-[#f5f1ff] rounded-b-2xl">View all matching conversations →</button>}
        </div>
      )}
    </div>
  );
