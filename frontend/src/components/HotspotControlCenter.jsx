...
function MemberAccessPanel({ routers }) {
  ...
  const submit=async e=>{e.preventDefault();setMessage('');const payload={username:form.username.trim(),password:form.password,router_id:Number(form.router_id),download_mbps:Number(form.download_mbps),upload_mbps:Number(form.upload_mbps)};if(!payload.username||payload.password.length<8||!payload.router_id||!(payload.download_mbps>0)||!(payload.upload_mbps>0)){setMessage('Choose a router and enter a username, 8+ character password, download and upload speeds.');return;}setBusy(true);try{await api.post('/billing-workspace/hotspot/members',payload);setForm({username:'',password:'',router_id:'',download_mbps:'',upload_mbps:''});setMessage('Member login created and synchronized to RADIUS.');load();}catch(err){setMessage(err.response?.data?.error||'Could not create member login.')}finally{setBusy(false)}};
