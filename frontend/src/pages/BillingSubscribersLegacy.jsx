...
function TypeGlyph({ kind }) { const content = kind === 'pppoe' ? <><circle cx="9" cy="9" r="3" /><path d="M3 20a6 6 0 0 1 12 0" /><circle cx="17" cy="11" r="2" /><path d="M14 20a4 4 0 0 1 7 0" /></> : kind === 'static' ? <><circle cx="12" cy="12" r="2" /><path d="M12 4v6m0 4v6M4 12h6m4 0h6" /></> : <><path d="M5 13.5a10 10 0 0 1 14 0" /><path d="M8 16.5a6 6 0 0 1 8 0" /><path d="M11 19.5a2 2 0 0 1 2 0" /></>; return <svg viewBox="0 0 24 24" className="h-6 w-6 fill-none stroke-current" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">{content}</svg>; }function MigrationCenter({ routers, plans, close, reload }) {
  ...
  const sourcePackages = [...new Set((batch?.rows || []).map((row) => row.normalized?.package_name).filter(Boolean))];
  ...
  const action = async (name, request) => {
    ...
    try {
      ...
      if (name === 'apply') await reload();
    } catch (requestError) {
      setLocalError(requestError.response?.data?.error || requestError.message || 'Migration action failed.');
    } finally { setBusy(''); }
  };
  ...
  const post = (name, suffix, body = {}) => action(name, () => api.post(`/billing-workspace/subscriber-migrations/${batch.id}/${suffix}`, body));
  const statusTone = batch?.status === 'handover_active' ? 'bg-emerald-100 text-emerald-800' : batch?.error_rows ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800';

  return <div className="fixed inset-0 z-[12000] flex items-end justify-center bg-slate-950/60 sm:items-center sm:p-5">
    <div className="max-h-[95vh] w-full max-w-5xl overflow-y-auto rounded-t-[2rem] bg-[#f7faf8] shadow-2xl sm:rounded-[2rem]">
      <header className="sticky top-0 z-10 flex items-start justify-between border-b border-emerald-100 bg-[#082c20] px-5 py-5 text-white sm:px-7">
        <div><p className="text-[10px] font-black uppercase tracking-[.25em] text-emerald-300">Safe client transfer</p><h3 className="mt-1 font-[Georgia,serif] text-2xl font-semibold">Migration Center</h3><p className="mt-1 text-xs text-emerald-100/80">Import first. Validate everything. Switch authentication only when you approve.</p></div>
      ...
      <div className="space-y-4 p-4 sm:p-7">
        ...
        <section className="grid gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-3">
          <label className="text-xs font-black text-slate-600">Legacy billing file
            <input type="file" accept=".csv,.xlsx,.xlsm" onChange={(event) => { setFile(event.target.files?.[0] || null); setBatch(null); }} className="mt-2 block w-full rounded-xl border border-slate-200 p-2 text-xs" />
          </label>
          <label className="text-xs font-black text-slate-600">Destination MikroTik
            <select value={routerId} onChange={(event) => { setRouterId(event.target.value); setBatch(null); }} className={field + ' mt-2'}><option value="">Select router</option>{routers.map((router) => <option key={router.id} value={router.id}>{router.name} · {router.last_status || 'status unknown'}</option>)}</select>
          </label>
          <label className="text-xs font-black text-slate-600">Source system
            <select value={sourceSystem} onChange={(event) => setSourceSystem(event.target.value)} className={field + ' mt-2'}><option value="generic">Generic CSV / Excel</option><option value="wispman">Wispman</option><option value="billnasi">Billnasi</option></select>
          </label>
          <div className="sm:col-span-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-emerald-50 p-3">
            <p className="max-w-2xl text-xs font-semibold text-emerald-900">Preview only reads the file and current MikroTik state. It does not modify subscribers, credentials, RADIUS, or active sessions.</p>
            <button type="button" disabled={busy || !file || !routerId} onClick={preview} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">{busy === 'preview' ? 'Validating…' : batch ? 'Run validation again' : 'Preview migration'}</button>
          </div>
        </section>

        {batch && <>
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-5">
            {[['Rows',batch.total_rows,'slate'],['Ready',batch.ready_rows,'emerald'],['Warnings',batch.warning_rows,'amber'],['Errors',batch.error_rows,'rose'],['Online matches',batch.summary?.online_matches || 0,'sky']].map(([label,value,tone]) => <div key={label} className={`rounded-2xl border border-${tone}-100 bg-${tone}-50 p-3`}><p className="text-[9px] font-black uppercase tracking-wider text-slate-500">{label}</p><p className="mt-1 text-2xl font-black text-slate-900">{value}</p></div>)}
          </section>

          {sourcePackages.length > 0 && <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><h4 className="font-black text-slate-900">Package mapping</h4><p className="mt-1 text-xs text-slate-500">Match every package name from the old billing file to its Polyizon package, then validate again.</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{sourcePackages.map((name) => <label key={name} className="text-xs font-bold text-slate-600">{name}<select value={packageMap[name] || ''} onChange={(event) => setPackageMap({ ...packageMap, [name]: event.target.value })} className={field + ' mt-1'}><option value="">Auto-match by name/profile</option>{plans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name}</option>)}</select></label>)}</div></section>}

          <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-3"><div><h4 className="font-black text-slate-900">Validation report</h4><p className="text-xs text-slate-500">Passwords are encrypted before this report is stored and are never displayed.</p></div><span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase ${statusTone}`}>{batch.status.replaceAll('_',' ')}</span></div>
            <div className="max-h-72 overflow-auto"><table className="w-full min-w-[760px] text-left text-xs"><thead className="sticky top-0 bg-slate-50 text-[9px] uppercase tracking-wider text-slate-400"><tr><th className="p-3">Row</th><th>Client</th><th>Username</th><th>Package</th><th>Expiry</th><th>Router match</th><th>Result</th></tr></thead><tbody className="divide-y divide-slate-100">{(batch.rows || []).map((row) => <tr key={row.id}><td className="p-3 font-bold">{row.row_number}</td><td>{row.normalized?.full_name}</td><td>{row.normalized?.username}</td><td>{row.normalized?.package_name || '—'}</td><td>{row.normalized?.expires_at ? new Date(row.normalized.expires_at).toLocaleString() : 'Missing'}</td><td>{row.matched_live_client_id ? 'Matched' : 'Not found'}</td><td className={row.validation_status === 'error' ? 'font-bold text-rose-600' : row.validation_status === 'warning' ? 'font-bold text-amber-600' : 'font-bold text-emerald-600'}><span>{row.validation_status}</span>{[...(row.errors || []),...(row.warnings || [])].map((message) => <span key={message} className="mt-1 block max-w-xs text-[10px] font-medium text-slate-500">{message}</span>)}</td></tr>)}</tbody></table></div>
          </section>

          <section className="rounded-3xl border border-emerald-200 bg-white p-4 shadow-sm"><div className="grid gap-4 lg:grid-cols-[1fr_auto]"><div><h4 className="font-black text-slate-900">Approval and zero-drop handover</h4><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-600">Apply imports clients and synchronizes RADIUS without touching the router. Prepare creates a disabled Polyizon RADIUS entry and records a rollback snapshot. Activate switches only after your exact confirmation and verifies that every active PPPoE session remained online.</p><input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder={batch.confirmation_phrase} className={field + ' mt-3 max-w-md font-mono'} /></div><div className="flex flex-wrap content-start gap-2 lg:max-w-sm lg:justify-end">
              {batch.status === 'validated' && <button type="button" disabled={busy || Number(batch.error_rows) > 0} onClick={() => post('apply','apply',{ confirmation })} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-40">{busy === 'apply' ? 'Synchronizing…' : 'Apply clients to Polyizon'}</button>}
              {batch.status === 'radius_ready' && <button type="button" disabled={busy} onClick={() => post('prepare','handover/prepare')} className="rounded-xl bg-sky-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-40">{busy === 'prepare' ? 'Preparing…' : 'Prepare router handover'}</button>}
              {batch.status === 'handover_prepared' && <button type="button" disabled={busy} onClick={() => post('activate','handover/activate',{ confirmation })} className="rounded-xl bg-amber-500 px-4 py-2.5 text-xs font-black text-amber-950 disabled:opacity-40">{busy === 'activate' ? 'Checking sessions…' : 'Activate Polyizon authentication'}</button>}
              {['handover_prepared','handover_active'].includes(batch.status) && <button type="button" disabled={busy} onClick={() => post('rollback','handover/rollback')} className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs font-black text-rose-700 disabled:opacity-40">{busy === 'rollback' ? 'Restoring…' : 'Restore previous router auth'}</button>}
            </div></div>
          </section>
        </>}
      </div>
...
export default function BillingSubscribers({ subscribers, items: sourceItems, networkClients = [], plans, hotspotPlans = [], routers = [], createOpen, setCreateOpen, search, setSearch, reload, setError, darkMode = false }) {
...
`}</style>
    ...
    {migrationOpen && <MigrationCenter routers={routers} plans={plans} reload={reload} close={() => setMigrationOpen(false)} />}
