  ...
  async function confirmDelivery() {
    ...
    try {
      if (deliveryTarget.type === 'draft') {
        const created = await createInvoice(true, { channe…3442 tokens truncated…                     {products.filter((product) => product.is_active !== false).map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}
                          </select>
