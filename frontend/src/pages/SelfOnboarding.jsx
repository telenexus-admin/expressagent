...
const initialForm = { business_name: '', owner_name: '', phone: '', email: '', location: '', service_interest: 'isp_support', connection_method: 'qr', consent_accepted: false, request_type: 'new_client', parent_client_id: '', agent_label: '' };
...
function ConnectionMethodChooser({ value, onChange }) {
  return <div className="rounded-[26px] border border-slate-200 bg-[#F8F8FD] p-4"><p className="text-sm font-black text-slate-950">Choose how you will connect WhatsApp</p><p className="mt-1 text-xs leading-5 text-slate-500">We create a separate secure connection for your chosen method.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => onChange('qr')} className={`rounded-2xl border p-4 text-left transition ${value === 'qr' ? 'border-[#3535FF] bg-[#E9E9FF]' : 'border-slate-200 bg-white'}`}><span className="text-xl">▦</span><span className="mt-2 block text-sm font-black text-slate-950">QR code</span><span className="mt-1 block text-xs leading-5 text-slate-500">Best when you can scan from a second phone.</span></button><button type="button" onClick={() => onChange('pairing_code')} className={`rounded-2xl border p-4 text-left transition ${value === 'pairing_code' ? 'border-[#3535FF] bg-[#E9E9FF]' : 'border-slate-200 bg-white'}`}><span className="text-xl">⌁</span><span className="mt-2 block text-sm font-black text-slate-950">Pairing code</span><span className="mt-1 block text-xs leading-5 text-slate-500">Use this when the phone being linked is the only phone available.</span></button></div><p className="mt-3 rounded-xl bg-white px-3 py-2 text-xs leading-5 text-slate-500">{value === 'qr' ? 'Example: use QR when WhatsApp is on one phone and you can scan with another phone.' : 'Example: use pairing code when you are holding the WhatsApp phone and cannot scan its screen with a second device.'}</p></div>;
}
...
export default function SelfOnboarding() {
  ...
  const isAdditionalAgent = searchParams.get('additionalAgent') === '1';
  const [introIndex, setIntroIndex] = useState(0);
  const [stage, setStage] = useState(isAdditionalAgent ? 'details' : 'intro');
  const [form, setForm] = useState(() => ({
    ...initialForm,
    business_name: searchParams.get('business') || '',
    email: searchParams.get('email') || '',
    request_type: isAdditionalAgent ? 'additional_agent' : 'new_client',
    parent_client_id: isAdditionalAgent ? (searchParams.get('clientId') || '') : '',
    agent_label: isAdditionalAgent ? 'Agent Two' : '',
  }));
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setConnectMethod(data.connection_method === 'pairing_code' ? 'pairing_code' : 'qr');
      setStage(data.status === 'connected' ? 'connected' : (data.qr_code || data.pairing_code ? 'qr' : (data.phone_verified ? 'qr' : 'otp')));
  ...
  if (stage === 'connected') return <div className="flex min-h-screen items-center justify-center bg-[#090916] p-5 text-white"><div className="w-full max-w-md rounded-[34px] border border-white/10 bg-white/[0.06] p-7 text-center shadow-2xl backdrop-blur"><div className="mx-auto mb-5 flex h-20 w-20 items-center justify-center rounded-full bg-emerald-400/15 text-4xl text-emerald-300">✓</div><p className="text-xs font-black tracking-[0.22em] text-emerald-300">WHATSAPP CONNECTED</p><h1 className="mt-4 text-3xl font-black">{isAdditionalAgent ? `${form.agent_label || 'New agent'} is connected.` : `You are in, ${session?.business_name || form.business_name}.`}</h1><p className="mt-4 text-sm leading-6 text-white/65">{isAdditionalAgent ? 'Your additional WhatsApp agent request has been received. Track review and activation status from Agent Configuration in your dashboard.' : 'Your connection request has been securely received. The Telenexus team will review your business and prepare your Nexa AI workspace.'}</p><div className="mt-7 rounded-2xl bg-white/10 px-5 py-4 text-sm text-white/70">We will contact you using the details submitted during onboarding.</div></div></div>;
  ...
  return <div className="min-h-screen bg-[#F6F7FF] px-5 py-6"><div className="mx-auto max-w-lg"><div className="mb-7 flex items-center justify-between"><NexaMiniBrand /><span className="rounded-full bg-[#E9E9FF] px-4 py-2 text-xs font-black text-[#3535FF]">Step 1 of 2</span></div><div className="rounded-[34px] bg-white p-6 shadow-xl shadow-indigo-100/70 sm:p-8"><p className="text-xs font-black tracking-[0.2em] text-[#3535FF]">{isAdditionalAgent ? 'ADD WHATSAPP AGENT' : 'SELF ONBOARDING'}</p><h1 className="mt-3 text-3xl font-black tracking-tight text-slate-950">{isAdditionalAgent ? 'Connect another WhatsApp number' : 'Tell us about your business'}</h1><p className="mt-2 text-sm leading-6 text-slate-500">{isAdditionalAgent ? 'This number will be reviewed by the onboarding team, then added as another agent in your dashboard.' : 'We use these details to prepare a secure Nexa workspace after you connect WhatsApp.'}</p><form onSubmit={submit} className="mt-7 space-y-4"><TextField label="Business / ISP name" value={form.business_name} onChange={(value) => update('business_name', value)} placeholder="e.g. SmartLink Fibre" /><TextField label="Your full name" value={form.owner_name} onChange={(value) => update('owner_name', value)} placeholder="Business owner or manager" />{isAdditionalAgent && <TextField label="Agent label" value={form.agent_label} onChange={(value) => update('agent_label', value)} placeholder="e.g. Agent Two, Sales line, Support line" />}<div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><TextField label="WhatsApp number" value={form.phone} onChange={(value) => update('phone', value)} placeholder="2547XXXXXXXX" /><TextField label="Email address" type="email" value={form.email} onChange={(value) => update('email', value)} placeholder="you@business.com" /></div><TextField label="Location" value={form.location} onChange={(value) => update('location', value)} placeholder="Town / area served" /><label className="block"><span className="mb-2 block text-xs font-bold text-slate-600">What would you like Nexa to help with?</span><select value={form.service_interest} onChange={(event) => update('service_interest', event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-[#F8F8FD] px-4 py-3.5 text-sm outline-none focus:border-[#3535FF]"><option value="isp_support">ISP customer support</option><option value="customer_support">General customer support</option><option value="sales_support">Leads and sales enquiries</option><option value="full_automation">Full automation setup</option></select></label><ConnectionMethodChooser value={form.connection_method} onChange={(value) => update("connection_method", value)} /><label className="flex gap-3 rounded-2xl bg-[#F6F7FF] p-4 text-xs leading-5 text-slate-600"><input type="checkbox" checked={form.consent_accepted} onChange={(event) => update('consent_accepted', event.target.checked)} className="mt-1 h-4 w-4 accent-[#3535FF]" /><span>I confirm that I control the WhatsApp number I will link and consent to Telenexus configuring Nexa AI after review.</span></label>{error && <div className="rounded-2xl bg-rose-50 p-4 text-sm text-rose-700">{error}</div>}<button type="submit" disabled={submitting || !form.consent_accepted} className="w-full rounded-2xl bg-[#3535FF] px-6 py-4 text-sm font-black text-white shadow-lg disabled:opacity-50">{submitting ? 'Preparing connection...' : 'Continue to WhatsApp connection →'}</button></form></div></div></div>;
}
