...
const initialForm = { business_name: '', owner_name: '', phone: '', email: '', location: '', service_interest: 'isp_support', consent_accepted: false, request_type: 'new_client', parent_client_id: '', agent_label: '' };
...
export default function SelfOnboarding() {
  ...
  return <div className="min-h-screen bg-[#F6F7FF] px-5 py-6"><div className="mx-auto max-w-lg"><div className="mb-7 flex items-center justify-between"><NexaMiniBrand /><span className="rounded-full bg-[#E9E9FF] px-4 py-2 text-xs font-black text-[#3535FF]">Step 1 of 2</span></div><div className="rounded-[34px] bg-white p-6 shadow-xl shadow-indigo-100/70 sm:p-8"><p className="text-xs font-black tracking-[0.2em] text-[#3535FF]">{isAdditionalAgent ? 'ADD WHATSAPP AGENT' : 'SELF ONBOARDING'}</p><h1 className="mt-3 text-3xl font-black tracking-tight text-slate-950">{isAdditionalAgent ? 'Connect another WhatsApp number' : 'Tell us about your business'}</h1><p className="mt-2 text-sm leading-6 text-slate-500">{isAdditionalAgent ? 'This number will be reviewed by the onboarding team, then added as another agent in your dashboard.' : 'We use these details to prepare a secure Nexa workspace after you connect WhatsApp.'}</p><form onSubmit={submit} className="mt-7 space-y-4"><TextField label="Business / ISP name" value={form.business_name} onChange={(value) => update('business_name', value)} placeholder="e.g. SmartLink Fibre" /><TextField label="Your full name" value={form.owner_name} onChange={(value) => update('owner_name', value)} placeholder="Business owner or manager" />{isAdditionalAgent && <TextField label="Agent label" value={form.agent_label} onChange={(value) => update('agent_label', value)} placeholder="e.g. Agent Two, Sales line, Support line" />}<div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><TextField label="WhatsApp number" value={form.phone} onChange={(value) => update('phone', value)} placeholder="2547XXXXXXXX" /><TextField label="Email address" type="email" value={form.email} onChange={(value) => update('email', value)} placeholder="you@business.com" /></div><TextField label="Location" value={form.location} onChange={(value) => update('location', value)} placeholder="Town / area served" /><label className="block"><span className="mb-2 block text-xs font-bold text-slate-600">What would you like Nexa to help with?</span><select value={form.service_interest} onChange={(event) => update('service_interest', event.target.value)} className="w-full rounded-2xl border border-slate-200 bg-[#F8F8FD] px-4 py-3.5 text-sm outline-none focus:border-[#3535FF]"><option value="isp_support">ISP customer support</option><option value="customer_support">General customer support</option><option value="sales_support">Leads and sales enquiries</option><option value="full_automation">Full automation setup</option></select></label><label className="flex gap-3 rounded-2xl bg-[#F6F7FF] p-4 text-xs leading-5 text-slate-600"><input type="checkbox" checked={form.consent_accepted} onChange={(event) => update('consent_accepted', event.target.checked)} className="mt-1 h-4 w-4 accent-[#3535FF]" /><span>I confirm that I control the WhatsApp number I will link and consent to Telenexus configuring Nexa AI after review.</span></label>{error && <div className="rounded-2xl bg-rose-50 p-4 text-sm text-rose-700">{error}</div>}<button type="submit" disabled={submitting || !form.consent_accepted} className="w-full rounded-2xl bg-[#3535FF] px-6 py-4 text-sm font-black text-white shadow-lg disabled:opacity-50">{submitting ? 'Preparing connection...' : 'Continue to WhatsApp connection →'}</button></form></div></div></div>;
}
