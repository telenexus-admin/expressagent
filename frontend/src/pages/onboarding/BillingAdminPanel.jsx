...
const ROUTES = ['dashboard', 'accounts', 'payments', 'create_account', 'admin_users'];
const navItems = [
  ['dashboard', 'Dashboard', '▦'],
...
function titleFor(route) {
  return {
    ...
    create_account: 'Create ISP account',
    admin_users: 'Master admin users',
  }[route];
}
...
export default function BillingAdminPanel() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [summary, accountList, payments, admins] = await Promise.all([
        api.get('/billing-operator/overview'),
        api.get('/billing-operator/accounts'),
        api.get('/settlements/operator/requests'),
        api.get('/admins'),
      ]);
      setOverview(summary.data);
      setAccounts(accountList.data);
      setPaymentRequests(payments.data?.requests || []);
  ...
  const updateAccount = async (account, payload) => {
    try {
      await api.patch(`/billing-operator/accounts/${account.id}`, payload);
      setNotice(`${account.name} was updated.`);
      await load();
    } catch (err) {
      setError(err.response?.data?.error || err.response?.data?.errors?.[0]?.msg || 'Could not update ISP account.');
      throw err;
    }
  };
  ...
  const extendSubscription = async (account, trial_ends_at) => {
    try {
      await api.post(`/billing-operator/accounts/${account.id}/extend`, { trial_ends_at });
      setNotice(`${account.name}'s subscription was extended.`);
      await load();
    } catch (err) {
      setError(err.response?.data?.error || err.response?.data?.errors?.[0]?.msg || 'Could not extend subscription.');
      throw err;
    }
  };
  ...
  const deleteAccount = async (account, confirm_name) => {
    try {
      await api.delete(`/billing-operator/accounts/${account.id}`, { data: { confirm_name } });
      setNotice(`${account.name} was deleted.`);
      await load();
    } catch (err) {
      setError(err.response?.data?.error || err.response?.data?.errors?.[0]?.msg || 'Could not delete ISP account.');
      throw err;
    }
  };

  const reviewPaymentRequest = async (request, decision) => {
    const ispName = request.client?.name || `ISP #${request.client?.id}`;
    const approving = decision === 'verified';
    const prompt = approving
      ? `Approve ${ispName}'s ${request.profile?.institution_name || 'bank'} destination ${request.profile?.account_number_masked || ''}?`
      : `Reject ${ispName}'s bank destination request?`;
    if (!window.confirm(prompt)) return;

    setReviewBusy(request.request_id);
    setError('');
    setNotice('');
    try {
      const { data } = await api.post(`/settlements/operator/${request.client.id}/review`, {
        decision,
        expected_updated_at: request.profile?.updated_at,
      });
      if (approving) {
        const emailStatus = data.email_notification?.status;
        setNotice(emailStatus === 'sent'
          ? `${ispName}'s bank destination was approved and Polyizon sent the approval email.`
          : `${ispName}'s bank destination was approved. The email notification status is ${emailStatus || 'unavailable'}.`);
      } else {
        setNotice(`${ispName}'s bank destination request was rejected.`);
      }
      await load();
    } catch (err) {
      setError(err.response?.data?.error || 'Could not review the payment request.');
    } finally {
      setReviewBusy(null);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#f5f6fc] text-[#121832]">
      ...
      <main className="mx-auto max-w-7xl p-4 pb-12 sm:p-6">
        ...
        {activeRoute === 'payments' && <PaymentRequests requests={paymentRequests} loading={loading} busyId={reviewBusy} onReview={reviewPaymentRequest} />}
...
function Dashboard({ total, recent, loading, openAccounts, openCreate }) {
  const stats = [
    ['Total ISPs', total.total_isps, 'bg-violet-50 text-violet-700'],
    ['Active ISPs', total.active_isps, 'bg-emerald-50 text-emerald-700'],
    ['Trial ISPs', total.trial_isps, 'bg-amber-50 text-amber-700'],
    ['Suspended', total.suspended_isps, 'bg-rose-50 text-rose-700'],
    ['Revenue', money(total.revenue), 'bg-sky-50 text-sky-700'],
    ['Plans', total.total_plans, 'bg-indigo-50 text-indigo-700'],
  ];
...
function CreateAccount({ onCreated }) {
  ...
  const submit = async (event) => {
    ...
    setSaving(true);
    try {
      const { data } = await api.post('/billing-operator/accounts', form);
      await onCreated(data.account.name, data.domain, data.domain_error);
    } catch (err) {
      const validation = err.response?.data?.errors?.[0]?.msg;
      setError(validation || err.response?.data?.error || 'Could not create the ISP account.');
    } finally {
      setSaving(false);
    }
  };
...
function MasterAdmins({ admins, loading, onCreated }) {
  ...
  const submit = async (e) => {
    ...
    setSaving(true);
    try {
      await api.post('/admins', { ...form, role: 'superadmin' });
      setForm({ name: '', email: '', password: '' });
      setOpen(false);
      await onCreated();
    } catch (err) {
      setError(err.response?.data?.errors?.[0]?.msg || err.response?.data?.error || 'Could not create master admin.');
    } finally {
      setSaving(false);
    }
  };
...
function DomainAutomation() {
  ...
  const [form, setForm] = useState({ cloudflare_zone_id: '', cloudflare_api_token: '', root_domain: 'polyizon.tech', target_domain: 'billing.polyizon.tech', proxied: true });
  const [open, setOpen] = useState(false);
  ...
  const loadSettings = async () => {
    try {
      const { data } = await api.get('/billing-operator/domains/settings');
      setSettings(data);
      setForm((current) => ({ ...current, cloudflare_zone_id: data.cloudflare_zone_id || '', root_domain: data.root_domain || 'polyizon.tech', target_domain: data.target_domain || 'billing.polyizon.tech', proxied: data.proxied !== false }));
    } catch (err) {
      setError(err.response?.data?.error || 'Could not load domain automation settings.');
    }
  };
  ...
  const save = async (event) => {
    ...
    setMessage('');
    try {
      const { data } = await api.put('/billing-operator/domains/settings', form);
      setSettings(data);
      setForm((current) => ({ ...current, cloudflare_api_token: '' }));
      setMessage('Settings saved. The API token is encrypted and shown only as masked.');
    } catch (err) {
      setError(err.response?.data?.error || 'Could not save Cloudflare settings.');
    } finally {
      setBusy(false);
    }
  };
  const verify = async () => {
    ...
    setMessage('');
    try {
      const { data } = await api.post('/billing-operator/domains/verify');
      setMessage(`Cloudflare verified. New ISP domains will point to ${data.target_domain}.`);
    } catch (err) {
