...
export class AriClient {
  constructor({ fetchImpl = fetch } = {}) { this.fetchImpl = fetchImpl; this.socket = null; }
  async request(path, options = {}) { const response = await this.fetchImpl(`${config.ariUrl}${path}`, { ...options, headers: { Authorization: authHeader(), ...(options.headers || {}) } }); const text = await response.text(); const body = text ? JSON.parse(text) : null; if (!response.ok) throw new Error(`ARI ${response.status}: ${JSON.stringify(body)}`); return body; }
  ...
  connect(onEvent) { const url = `${config.ariUrl.replace(/^http/, 'ws')}/events?app=${encodeURIComponent(config.ariApp)}`; this.socket = new WebSocket(url, { headers: { Authorization: authHeader() } }); this.socket.on('message', (data) => { try { onEvent(JSON.parse(data)); } catch {} }); return this.socket; }
}